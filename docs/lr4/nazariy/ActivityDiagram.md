...
flowchart TD
    ...
    SchedulerTrigger["Планувальник спрацьовує"] --> CheckDueNotifications["NotificationManager.checkDueNotifications()"]
    ...
    IncrementRetry --> ChangeStateToPendingRetry["Notification.state = PENDING (для повтору)"]
    ChangeStateToPendingRetry --> ManagerReschedules["NotificationManager переплановує"]
    ...
    UserNotificationReceived["Користувач отримав сповіщення"] --> UserAction{"Дія користувача?"}
