...
    %% Спрощений процес створення сповіщення без Factory та Builder
    PC->>NM: getInstance().schedulePaymentReminder(newPayment)
    activate NM
    %% NM тепер відповідає за створення та конфігурацію безпосередньо
    NM->>N: new Notification(id, PAYMENT_REMINDER)
    activate N
    NM->>N: setMessage("Reminder for " + newPayment.getDescription())
    NM->>N: setTriggerDate(calculateReminderDate(newPayment.getDueDate(), userPrefs))
    NM->>N: setState(NotificationState.SCHEDULED)
    note right of N: State: SCHEDULED
    N-->>NM: paymentReminderNotification
    deactivate N
